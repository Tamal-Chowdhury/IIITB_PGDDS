...
#	Import the Bollywood data set in Rstudio in a variable named bollywood
  ...
  bollywood <- read.csv(file = "bollywood.csv", header = TRUE, sep = ",",
                        dec = ".", strip.white = TRUE, blank.lines.skip = TRUE)
  View(bollywood)
...
# Store the movie name in variable named top_movie
  ...
  top_movie <- bollywood$Movie[which(bollywood$Tcollection == max(bollywood$Tcollection))]
...
# Store the movie name in variable named top_2_movie
  ...
  top_2_movie <- bollywood$Movie[which(bollywood$Tcollection == sort(bollywood$Tcollection, decreasing = TRUE)[2])]
...
bollywood$Verdict <- as.character(bollywood$Verdict)
verdict <- function(all_cols,verd){
    sum(all_cols$Verdict == verd)
}

verdict_avg = sapply(list(bollywood), verdict, "Average")
verdict_flop = sapply(list(bollywood), verdict, "Flop")
verdict_hit = sapply(list(bollywood), verdict, "Hit")
...
# Store the names of 4 movies in same sequence in movie_result vector
 # bollywood$Movie <- as.character(bollywood$Movie)
  movie_result <- c(bollywood$Movie[which.max(bollywood$Ocollection)],
                    bollywood$Movie[which.max(bollywood$Wcollection)],
                    bollywood$Movie[which.max(bollywood$Fwcollection)],
